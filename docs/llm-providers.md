...
- 本次 SSRF 收紧后，`base_url` 会拒绝空白/控制字符、反斜杠路径、`userinfo@host` 形式及不安全解析输入；常见兼容地址仍可使用（如 `https://api.openai.com/v1`、`https://api.deepseek.com`、`https://api.siliconflow.cn/v1`、`https://dashscope.aliyuncs.com/compatible-mode/v1`、`http://127.0.0.1:11434`）。若出现 `invalid_url`，请清理该 `LLM_<CHANNEL>_BASE_URL`（设为空或移除），重试保存后再按账号权限运行测试。
