...
- [新功能] 新增 `scripts/deploy_vps.sh` 一键部署脚本，在 Ubuntu VPS 上自动完成虚拟环境、前端构建、systemd 常驻服务（服务名 `dsa`）、Nginx 反向代理与 Let's Encrypt HTTPS 配置；支持与同服务器上其他项目共存，部署前强制校验 `.env` 已设 `ADMIN_AUTH_ENABLED=true`，详见 `docs/DEPLOY.md` 方案五。
...
- [改进] 短线荐股新增盘中模式并设为默认（`SWING_PICKS_MORNING_TIME` 默认 09:00 → 10:00）：口径由触发时刻的市场阶段决定（`trading_calendar.infer_market_phase`，日历库不可用时按本地时钟兜底）——开盘前触发保持盘前口径，连续竞价/午休/尾盘竞价触发为盘中口径，收盘后触发（如宕机后补跑）跳过当日荐股并标记已处理；盘中快照的涨跌幅即今日实时涨幅（筛"今天正在走强"而非"昨天涨得好"，缓解盘前荐股当日开盘走坏的痛点）；成交额/换手率门槛按已交易时长动态折算（10:00≈25%、14:00≈75%，量比天生盘中口径不折算），涨幅 1% 下限在开盘前 30 分钟同比例放宽；新增当日走势健康筛选（现价≥今开、距当日高点回撤≤3%、今开≥昨收×98%，今开/最高/昨收任一缺失即拒绝，安全闸宁可错杀）；技术面筛选统一剔除日线中当天未走完的半根K线（盘中以快照现价为最新价参与均线与5日涨幅计算）；盘中荐股当下直接以现价确定买入参考价与止盈/止损价并立即进入监控（不再等开盘价回填），推送直接给出买入/止盈/止损三个价位；买入定价逻辑抽为 service/worker 共享函数，A股交易时段常量收拢为单一定义；worker 在触发荐股的同一轮跳过持仓监控（避免行情缓存滞后误发卖出提醒）；附纯逻辑单元测试（动态折算/健康筛选/半根K线/即时定价/口径解析/已交易时长）。
